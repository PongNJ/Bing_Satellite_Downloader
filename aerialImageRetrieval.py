"""
This module is applied from https://github.com/llgeek/Satellite-Aerial-Image-Retrieval
by __author__ = Linlin Chen
__email__ = lchen96@hawk.iit.edu

The original code is commented out
I made some changes to the original code to make it work with the current project.
Here's the list
1. 
...
class AerialImageRetrieval(object):
    ...
    def max_resolution_imagery_retrieval(self):
        """*** Note: This method is modified from the original code ***
        I had made some changes to save satellite images in the output folder 
        ...
        for levl in range(TileSystem.MAXLEVEL, 0, -1):
            ...
            filename = os.path.join(self.tgtfolder, '{}_{}.jpeg'.format(levl,"_"+timestamp))
            print(f"Finish the aerial image retrieval, stored the image as {filename}")
            retrieve_image.save(filename)
            return True
        return False    
